...
const GENERIC_LEGACY_JS_DIR = BUILD_DIR + "generic-legacy-js/";
...
// 创建CommonJS格式的webpack配置
function createWebpackConfigJS(defines, output, options = {}) {
  const config = createWebpackConfig(defines, output, options);
  // 修改为CommonJS格式
...
function createMainBundleJS(defines) {
  const mainFileConfig = createWebpackConfigJS(defines, {
...
function createWorkerBundleJS(defines) {
  const workerFileConfig = createWebpackConfigJS(defines, {
...
function createSandboxBundleJS(defines, extraOptions = undefined) {
  const scriptingPath = TMP_DIR + "pdf.scripting.mjs";
  // Insert the source as a string to be `eval`-ed in the sandbox.
  const sandboxDefines = {
    ...defines,
  ...
  const sandboxFileConfig = createWebpackConfigJS(
    sandboxDefines,
    {
      filename: sandboxDefines.MINIFIED
        ? "pdf.sandbox.min.js"
...
function createWebBundleJS(defines, options) {
  const viewerFileConfig = createWebpackConfigJS(
    defines,
    {
      filename: "viewer.js",
      library: {
...
function buildGenericJS(defines, dir) {
  fs.rmSync(dir, { recursive: true, force: true });

  return ordered([
    createMainBundleJS(defines).pipe(gulp.dest(dir + "web")),
    createWorkerBundleJS(defines).pipe(gulp.dest(dir + "web")),
    createSandboxBundleJS(defines).pipe(gulp.dest(dir + "web")),
    createWebBundleJS(defines, {
      defaultPreferencesDir: defines.SKIP_BABEL
        ? "generic/"
        : "generic-legacy/",
    }).pipe(gulp.dest(dir + "web")),
...
// 新的gulp任务：生成不包含mjs结构的旧版本浏览器兼容包
gulp.task(
  "generic-legacy-js",
  gulp.series(
    createBuildNumber,
    "locale",
    function scriptingGenericLegacyJS() {
      const defines = { ...DEFINES, GENERIC: true, SKIP_BABEL: false, LEGACY_JS: true };
      return ordered([
        buildDefaultPreferences(defines, "generic-legacy/"),
        createTemporaryScriptingBundle(defines),
      ]);
    },
    async function prefsGenericLegacyJS() {
      await parseDefaultPreferences("generic-legacy/");
    },
    function createGenericLegacyJS() {
      console.log();
      console.log("### Creating generic (legacy) viewer with JS format");
      const defines = { ...DEFINES, GENERIC: true, SKIP_BABEL: false, LEGACY_JS: true };

      return buildGenericJS(defines, GENERIC_LEGACY_JS_DIR);
    }
  )
);
